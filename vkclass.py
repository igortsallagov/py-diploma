...
class VKUser:
    ...
    def __init__(self, input_id, input_token):
        user_id = str(input_id)
        if user_id.isdigit() is False:
            params_init = dict(access_token=input_token, user_ids=input_id, v=VERSION)
    ...
    def get_friends(self, input_token):
        method_url = f'{API_URL}/friends.get'
        params_get_friends = dict(access_token=input_token, user_id=self.user_id, v=VERSION)
        result = requests.get(method_url, params_get_friends).json()
        return result

    def get_groups(self, input_token):
        method_url = f'{API_URL}/groups.get'
        params_get_groups = dict(access_token=input_token, user_id=self.user_id, count=1000, v=VERSION)
        result = requests.get(method_url, params_get_groups).json()
        return result

    def get_groups_data(self, group_ids, input_token):
        method_url = f'{API_URL}/groups.getById'
        params_groups_data = dict(access_token=input_token, fields='members_count', v=VERSION)
